...
if __name__ == '__main__':
    ...
    rs = ReplicationSender('{0}_sender@{1}'.format(config.HOST_NAME.lower(), config.HOST_SPADE_IP), 'secret')
    rs.start()
    rs.setDebugToScreen()

    rr = ReplicationReceiver('{0}_receiver@{1}'.format(config.HOST_NAME.lower(), config.HOST_SPADE_IP), 'secret')
    #rr.start()
    #rr.setDebugToScreen()
